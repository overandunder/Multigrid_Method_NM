...
implicit none
    ...
    integer, parameter :: Ntot=64, nlev = 5
    real(rprec), parameter :: errmax = 1.0e-3
    real(rprec), parameter :: wgs = 1.0 !1.8
    integer, parameter :: pr=100  !how often to write u to file
...
! Cycle
    do while (error .gt. errmax)
        ...
        !Write u to file every pr timesteps
            if (mod(loop,pr).eq.0) then
                call write_u(loop)
            endif
...
real(rprec), pointer, dimension(:,:) :: p_u => null()
    ...
    do m=n_i,n_f
        ...
        if (m.ne.1) call itsolve(m,all_levels_t%level_t(m)%N)

        if (m .ne. n_f) then
            ! Calculate RHSlow which will be used by next-lowest level
            call residual(m,all_levels_t%level_t(m)%N)

            !p_rhslow Coarsen RHSlow @m and set it to RHS @(m-1)
            call restriction(m)
        endif
    enddo
...
integer :: m
    ...
    do m=n_f,n_i+1,-1
        ...
        if (m.ne.1) call itsolve(m-1,all_levels_t%level_t(m-1)%N)
...
!*************************************************************
subroutine itsolve_u()
!*************************************************************
! 
!  This subroutine performs one iteration of Gauss-Seidel 
!  on the array 'u' for level '1' with size 'Ntot'^2 and source
!  term given by 0
!
integer :: i, j
real(rprec), pointer, dimension(:,:) :: p_u => null()
real(rprec) :: dxsq
    ...
    p_u => all_levels_t%u   
    dxsq = all_levels_t%level_t(1)%dx2  

    ! Perform iteration
        do j=1,Ntot
        do i=1,Ntot
            p_u(i,j) = wgs*0.25*( p_u(i-1,j)+p_u(i+1,j)+p_u(i,j-1)+p_u(i,j+1) ) + (1.-wgs)*p_u(i,j)
...
real(rprec) :: dxsq
    ...
    ! Perform iteration
        do j=1,Ntmp
        do i=1,Ntmp
            p_e(i,j) = wgs*0.25*( p_e(i-1,j)+p_e(i+1,j)+p_e(i,j-1)+p_e(i,j+1) - dxsq*p_s(i+(j-1)*Ntmp) ) + (1.-wgs)*p_e(i,j)
        enddo  
        enddo
